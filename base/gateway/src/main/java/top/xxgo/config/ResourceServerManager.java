...
public class ResourceServerManager implements ReactiveAuthorizationManager<AuthorizationContext> {
    ...
    public Mono<AuthorizationDecision> check(Mono<Authentication> mono, AuthorizationContext authorizationContext) {
        ...
        if (pathMatcher.match(GlobalConstant.APP_API_PATTERN, path)) {
            ...
            if (StrUtil.isNotBlank(token) && token.startsWith(AuthConstant.AUTHORIZATION_PREFIX)) {
                return Mono.just(new AuthorizationDecision(true));
            } else {
        ...
        Mono<AuthorizationDecision> authorizationDecisionMono = mono
                ...
                .any(authority -> {
                    ...
                    String role = authority.substring(AuthConstant.AUTHORITY_PREFIX.length()); // 角色编码 ROOT
